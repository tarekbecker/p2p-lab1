 ...
 * 
 */
public class BroadcastNode extends AbstractOverlayNode implements
		TransMessageListener, ChatNode {
	...
	public void initialize() {
		...
		try {
			udp = getAndBindUDP(net.getLocalInetAddress(), PORT_UDP);
			if (USE_TCP_FOR_UNICAST) {
				tcp = getAndBindTCP(net.getLocalInetAddress(), PORT_TCP);
			}
		} catch (ProtocolNotAvailableException e) {
			throw new AssertionError("Transport component not available!");
		}
		ownOverlayContact = new BasicOverlayContact(getHost().getId(),
				udp.getTransInfo());
	...
	public void messageArrived(Message msg, TransInfo sender, int commID) {
		if (msg instanceof BroadcastMessage) {
			...
			case BroadcastMessage.TYPE_TEST:
				System.out.println("Message Arrived: " + msg.toString()
						+ " from " + sender.toString());
				break;
			case BroadcastMessage.TYPE_CHAT:
				chatListener.receivedMessage(bMsg.getMessage(), bMsg
						.getSender().getNodeID());
				knownContacts.put(bMsg.getSender().getNodeID(),
						bMsg.getSender());
				break;
			default:
			...
			chatListener.receivedMessage(uMsg.getMessage(), uMsg.getSender()
					.getNodeID());
			knownContacts.put(uMsg.getSender().getNodeID(), uMsg.getSender());
			// send ACK
			if (!USE_TCP_FOR_UNICAST) {
				AckMessage ack = new AckMessage(getLocalOverlayContact(),
						uMsg.getSender());
	...
	public void sendChatMessage(String message, UniqueID to) {
		if (to == null || !knownContacts.containsKey(to)) {
			BroadcastMessage bMsg = new BroadcastMessage(
					getLocalOverlayContact(), message,
					BroadcastMessage.TYPE_CHAT);
			udp.send(bMsg, udp.getNetInterface().getBroadcastAddress(),
					PORT_UDP);
		} else {
			UnicastMessage bMsg = new UnicastMessage(getLocalOverlayContact(),
					knownContacts.get(to), message);

			// TCP-variant (no send-and-wait needed)
			if (USE_TCP_FOR_UNICAST) {
				tcp.send(bMsg, knownContacts.get(to).getTransInfo().getNetId(),
						knownContacts.get(to).getTransInfo().getPort());
			} else {
				udp.sendAndWait(bMsg, knownContacts.get(to).getTransInfo()
						.getNetId(), knownContacts.get(to).getTransInfo()
						.getPort(), new TransMessageCallback() {
					@Override
					public void receive(Message reply, TransInfo source,
							int commId) {
	...
	protected void doBroadcastAndReschedule(final long after) {
		...
		System.out.println("BroadcastNode: Sending Broadcast from "
				+ udp.getNetInterface().getLocalInetAddress());
		udp.send(new BroadcastMessage(getLocalOverlayContact(), "",
				BroadcastMessage.TYPE_TEST), udp.getNetInterface()
				.getBroadcastAddress(), PORT_UDP);
	}
	...
	public static class UnicastMessage extends AbstractOverlayMessage {
		...
		public UnicastMessage(OverlayContact sender, OverlayContact receiver,
				String message) {
